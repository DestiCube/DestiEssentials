...
@DuckCommand(
        ...
        usageARGS = "(name)",
        permissions = {})
public class TeleportRequest extends DestiCommand {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 0) return p.sendMessage(USAGE);
            ...
            OfflinePlayer tempPlayer = Bukkit.getOfflinePlayer(args[0]);
            switch (p.sendTeleportRequest(tempPlayer, RequestType.TO)) {
                ...
                case SUCCESS -> {
                    p.sendMessage(tl("teleportRequestSender", args[0]));
                    Bukkit.getPlayer(tempPlayer.getUniqueId()).sendMessage(tl("teleportRequestReciever", p.getNickName()));
                }
