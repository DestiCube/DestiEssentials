...
@DuckCommand(
        ...
        aliases = {"seeinv", "inventorysee"},
        usageARGS = "(player)",
        permissions = {"desticore.invsee"})
public class InvSee extends DestiCommand {

    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        DestiPlayer p = player(((Player) sender));
        if (!p.hasPermission("desticore.invsee")) return p.sendMessage(NO_PERMISSIONS);
        if (args.length == 0) p.sendMessage(USAGE);
        else {
            Player target = Bukkit.getPlayer(args[0]);
            if (!target.isOnline()) p.sendMessage(tl("playerNotOnline"));
