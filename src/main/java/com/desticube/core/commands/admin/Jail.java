...
@DuckCommand(
        command = "jail",
        description = "Jail a bitch",
        aliases = {},
        usageARGS = "(player) (jail)",
        permissions = {"desticore.jail"})
public class Jail extends DestiCommand {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length <= 1) p.sendMessage(USAGE);
            ...
            switch (SERVER.jail(Bukkit.getPlayer(args[0]), args[1])) {
                case SUCCESS -> {
                    p.sendMessage(tl("jailedPlayer", args[0], args[1]));
                }
                case NO_NAME -> {
                    p.sendMessage(tl("jailDoesntExist", args[1]));
                }
                case PLAYER_NOT_ONLINE -> {
                    p.sendMessage(tl("playerNotOnline", args[1]));
                }
                case ALREADY_DONE -> {
                    p.sendMessage(tl("playerAlreadyJailed", args[1]));
                }
                default -> {
                }
            }
    ...
    public List<String> onTabComplete(CommandSender sender, Command cmd, String label,
        ...
        if (args.length == 1) {
            ...
            StringUtil.copyPartialMatches(args[1], jails, newJails);
            return newJails;
        } else {
