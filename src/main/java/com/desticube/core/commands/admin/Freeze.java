...
@DuckCommand(
        ...
        aliases = {},
        usageARGS = "(player)",
        permissions = {"desticore.freeze"})
public class Freeze extends DestiCommand {

    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        DestiPlayer p = player(((Player) sender));
        if (!p.hasPermission("desticore.freeze")) return p.sendMessage(NO_PERMISSIONS);
        if (args.length == 0) p.sendMessage(USAGE);
        else {
            switch (SERVER.freeze(Bukkit.getPlayer(args[0]))) {
                case SUCCESS -> p.sendMessage(tl("playerFrozen", args[0]));
                case PLAYER_NOT_ONLINE -> p.sendMessage(tl("playerNotOnline"));
                case ALREADY_DONE -> p.sendMessage(tl("playerAlreadyFrozen"));
                default -> {
                }
            }
        }
